module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!user.roles.has(role.id)) return message.channel.send("Este usuario no tiene este rol")
        try {
        ...
        return message.channel.send(`Se ha removido el rol **${role.name}** de <@${user.id}>`)
    }
}