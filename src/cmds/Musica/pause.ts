...
export default {
    ...
    run: async ({ client, message }: {
        ...
        if (queue.isPaused()) { queue.resume(); return (message.channel as TextChannel).send("Se ha reanudado la canción") }
        queue.pause(); return (message.channel as TextChannel).send("Se ha pausado la canción")
    }
}