...
import { joinVoiceChannel, createAudioPlayer, createAudioResource, StreamType, AudioPlayerStatus, getVoiceConnection, VoiceConnectionStatus, entersState } from '@discordjs/voice'
import musicData, { Looped, Song } from './musicData'
...
async function playSong(queue: Song[], message: Message) {
    ...
    if (queue[0].provider === "Youtube" || musicData.server[message.guild?.id ?? ""].looped[0]) {
        ...
        musicData.server[message.guild?.id ?? ""].songDispatcher!.on(AudioPlayerStatus.Playing, async () => {
            ...
            if (musicData.server[message.guild?.id ?? ""].unPaused == true) {
                ...
                const embed = await (message.channel as TextChannel).send({ embeds: [videoEmbed] })
            ...
            .on('error', async e => {
                (message.channel as TextChannel).send('No se puede escuchar esa canción');
                musicData.server[message.guild?.id ?? ""].queue.length = 0;
                musicData.server[message.guild?.id ?? ""].isPlaying = null;
                musicData.server[message.guild?.id ?? ""].pause = false
