...
module.exports = async (oldRole, newRole) => {
    ...
    Guild.findOne({ guildID: newRole.guild.id }).then(async doc => {
        ...
        if (newperm == true) embed.addField("Permisos Agregados", `${getnewperm[0].new.toArray().join(", ")}`)
        if (removeperms == true) embed.addField("Permisos Removidos", `${getremoveperm[0].old.toArray().join(", ")}`)
